...
// delete user
function deleteissuebook($conn, $id)
{
  $sql = "delete from issuebooks where id =$id";
  return $conn->query($sql);
}

// Update user status
function updateissuebooks($conn, $id, $status)
{
  $sql = "UPDATE issuebooks set is_return = '$status' where id =$id";
  $result = $conn->query($sql);
  return $result;
}
